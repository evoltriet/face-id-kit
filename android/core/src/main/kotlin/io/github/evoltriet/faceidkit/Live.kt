...
class TrackConsensus(private val window: Int = 6, private val required: Int = 4,
    ...
    var diagnostics: Map<Long, ConsensusDiagnostics> = emptyMap(); private set
    private var pendingReset: String? = "session_start"
    fun reset(reason: String = "session_reset") { tracks.clear(); diagnostics = emptyMap(); pendingReset = reason }
    ...
    fun update(boxes: List<Box>, identities: List<String?>, now: Double = System.nanoTime() / 1e9): List<Pair<Long, Boolean>> {
        ...
        val expired = tracks.entries.removeAll { now - it.value.seen >= expirySeconds }
        val currentDiagnostics = linkedMapOf<Long, ConsensusDiagnostics>()
        ...
        val result = boxes.indices.map { i ->
            ...
            var reason = if (choices.any { it in ambiguous }) "ambiguous_association" else
                if (choices.isEmpty()) pendingReset ?: if (expired) "track_expired" else "new_spatial_track" else null
            ...
            if (track.history.isNotEmpty() && identity != null && track.history.last() != null && track.history.last() != identity) {
                track.history.clear(); reason = "identity_changed"
            }
            track.box = boxes[i]; track.seen = now; append(track, identity); used.add(id)
            currentDiagnostics[id] = ConsensusDiagnostics(if (identity == null) 0 else track.history.count { it == identity },
                track.history.size, required, window, reason)
        ...
        if (boxes.isNotEmpty()) pendingReset = null
        return result
    }
}
class LiveSession(private val faces: FaceIdentifier, private val maxFaces: Int = 4,
    ...
    @Synchronized fun update(image: BgrImage, now: Double = System.nanoTime() / 1e9): List<LiveResult> {
        ...
        return results.zip(tracks).map { (r, track) -> LiveResult(track.first, r.first, r.second, track.second,
            tracker.diagnostics[track.first] ?: ConsensusDiagnostics()) }
    }
}
