    ...
    @Test fun syntheticFaceAtOrdinaryAndCloseCameraSizes() {
        ...
        try {
            for (edge in listOf(320,1280)) {
                ...
                val image=BgrImage(edge,edge,bgr)
                assertTrue(backend.locate(image,emptyList()).isEmpty())
                assertEquals(0,backend.stats.passes)
                assertTrue(backend.embed(image,emptyList()).isEmpty())
                val whole=Box(0.0,0.0,edge.toDouble(),edge.toDouble())
                val locations=backend.locate(image,listOf(whole))
                assertEquals(1,locations.size)
                val staged=backend.embed(image,locations).single()
                assertTrue(galleryScore(face.embedding,listOf(staged.embedding))>.999)
                val changed=BgrImage(edge,edge,bgr.copyOf().apply {this[0]=(this[0].toInt() xor 1).toByte()})
                try {backend.embed(changed,locations);fail("Stale source accepted")} catch(_: IllegalArgumentException) {}
                val corner=backend.locate(image,listOf(Box(0.0,0.0,edge*.12,edge*.12)))
                assertTrue("Excluded face must not yield an embedding",backend.embed(image,corner).isEmpty())
